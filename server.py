...
{% if result %}
  ...
  {% for i,deck in enumerate(result["decks"],1) %}
    ...
    {% if deck.get("avg_elixir") %}<p>Média de Elixir: {{deck["avg_elixir"]}}</p>{% endif %}
    {% if deck.get("evolved_cards") %}<p>Evoluções (<=2): {{", ".join(deck["evolved_cards"])}}</p>{% endif %}
  {% endfor %}
{% endif %}
<p><a href="/">voltar</a></p>
"""
...
def _prepare_decks_output(player_id, result):
    ...
    evo_owned = [n for n,info in (get_player(player_id).get("cards_owned") or {}).items() if info.get("evolution")]

    decks = []
    for d in result.get("decks", []):
        raw = d.get("cards", [])[:8]
        fixed = validate_cards(raw, cards_idx)
        evos = clamp_evolutions(d.get("evolved_cards", []), evo_owned, max_per_deck=2)
...
def build_decks_form():
    ...
    if not player_id or not prompt:
        return "player_id e prompt são obrigatórios", 400

    player = get_player(player_id)
    if not player: return f"player {player_id} não encontrado", 404
    cards = get_cards()
    meta = get_meta()
    if not cards: return "Sem cards cache. Vá em /admin e clique 'Atualizar cartas'.", 400
    if not meta or not meta.get("archetypes"): return "Sem meta. Vá em /admin e cole o meta.", 400
...
def import_form():
    ...
    if request.method == "POST":
        api_key = _require_api_key()
        pid = request.form.get("player_id","").strip().lower()
...
def players_import_tag():
    ...
    pid = data.get("player_id","").strip().lower()
...
def players_add_card():
    ...
    player.setdefault("cards_owned", {})[name] = {"level": level, "evolution": evolution}
